
# Teoria da Computação
![img_tec](https://github.com/hertonnn/UDESC_Ciencia_da_Computacao/blob/master/utils/img/img_tec.jpg)

A teoria da computação é um subcampo da ciência da computação e matemática que busca determinar quais problemas podem ser computados em um dado modelo de computação. A computação pode ser definida como a solução de um problema ou, formalmente, o cálculo de uma função por meio de um algoritmo. Apesar de intuitivo na história humana, o conceito de execução de uma tarefa com passos finitos a fim de se obter um resultado, ou seja, um algoritmo, não possuía uma definição formal até a conceituação do modelo de Máquina Universal de Turing.

#### [Why study theory of computation?](https://www.youtube.com/watch?v=SV57Yv8BXBc)

### Links pertinentes 📌

- [O Sipser!](https://fuuu.be/polytech/INFOF408/Introduction-To-The-Theory-Of-Computation-Michael-Sipser.pdf)
- [Algumas respostas do Sipser. Livro que a disciplina irá se basear](https://moodle.joinville.udesc.br/pluginfile.php/368582/mod_resource/content/1/Solution-Manual-Introduction-to-the-Theory-of-Computation-Sipser.pdf)
- [Vídeos Mágicos do Sipser](https://ocw.mit.edu/courses/18-404j-theory-of-computation-fall-2020/video_galleries/video-lectures/)
- [Simulador de máquina de turing](http://morphett.info/turing/turing.html)
- [P versus NP](http://www.claymath.org/millennium/p-vs-np/)

### Links interessantes 🤓

- [O Mapa da Matemática](https://www.youtube.com/watch?v=OmJ-4B-mS-Y&t=2s)
- [O Hotel de Hilbert](http://ed.ted.com/lessons/the-infinite-hotel-paradox-jeff-dekofsky)
- [Há infinitos maiores que outros infinitos](https://youtu.be/dEOBDIyz0BU)
- [O Problema da Parada](https://www.youtube.com/watch?v=92WHN-pAFCs)

## Máquinas de Turing

**Definição informal:**

**1 -** Fita infinita dividida em células, cada uma contendo um símbolo de um alfabeto finito (incluindo um símbolo branco "¬"). Células não escritas são preenchidas com "¬".

**2 -** Cabeçote que lê, escreve e se move para esquerda (←) ou direita (→).

**3 -** Registrador de estados finitos, iniciando em um estado inicial definido.

**4 -** Tabela de ação (função de transição) que determina:

- Qual símbolo escrever,

- Como mover o cabeçote,

- Qual novo estado assumir,com base no símbolo lido e no estado atual. Se não houver entrada para a combinação, a máquina para.


**Propositions as Types**

O artigo "Propositions as Types" (Proposições como Tipos) de Philip Wadler explora a profunda conexão entre lógica e computação, mostrando como proposições lógicas correspondem a tipos em linguagens de programação, e provas correspondem a programas. 

["Propositions as Types" by Philip Wadler | História da computação](https://www.youtube.com/watch?v=IOiZatlZtGU)

**Indecibilidade**

...

**Complexidade de tempo**

...

### Video aulas da professora karina


- [Videoaula 01 - Máquinas de Turing com Movimento Estacionário](https://youtu.be/2RrV5BPKVMw)

- [Videoaula 02 - Máquinas de Turing com Fita Duplamente Infinita](https://youtu.be/OEU2PQwrny8)

- [Videoaula 03 - Máquinas de Turing Multifita](https://youtu.be/vyys4_Udl1Q)


- [Videoaula 04 - Máquinas de Turing Não Determinísticas](https://youtu.be/JlRPwOtNmqw)

- [Videoaula 05 - Codificação de Máquinas de Turing](https://youtu.be/oE4JTddb-_c)

- [Videoaula 06 - Linguagens Decidíveis](https://youtu.be/-wDowdq_AYo)

- [Videoaula 07 - Linguagens Co-reconhecíveis](https://youtu.be/Z2LGQ25A1R4)



### Sobre o Professor
Karina Girardi Roggia

Possui Doutorado em Matemática (tese em Lógica) pelo Instituto Superior Técnico em Lisboa, Portugal (2012). É Bacharel e Mestre em Ciência da Computação pela Universidade Federal do Rio Grande do Sul (2002 e 2005) com dissertação na área de Fundamentos da Computação. Recebeu Láurea Acadêmica pelo desempenho na graduação (obtendo nota máxima em 80% dos créditos). Foi membro do SQIG, vinculado ao Instituto de Telecomunicações em Portugal e do Instituto i3G - Instituto de Governo Eletrônico, Inteligências e Sistemas onde realizou estágio pós-doutoral entre 2013 e 2014 em projeto de buscas jurídicas multilíngues. Atualmente é professora adjunta na Universidade do Estado de Santa Catarina na área de Fundamentos da Computação, ministrando as disciplinas como Teoria da Computação e Matemática Discreta. Principais tópicos de interesse em pesquisa: assistentes de provas, lógicas não-clássicas e teoria das categorias.
### Email:
karina.roggia@udesc.br 
### Lattes:
 http://lattes.cnpq.br/7786281602726587
 










## Trabalhos Acadêmicos Relacionados (UDESC)

Abaixo estão alguns trabalhos acadêmicos desenvolvidos na UDESC que se relacionam com o conteúdo desta disciplina:

- **Formalização do Símbolo de Legendre em Coq**
  - *Autor(es)/Ano:* Bruno Rafael dos Santos (Orientadora: Karina G. Roggia) / 2024
  - *Link:* [https://repositorio.udesc.br/entities/publication/1808a890-589e-4f31-bb6f-cfe316b995cd](https://repositorio.udesc.br/entities/publication/1808a890-589e-4f31-bb6f-cfe316b995cd)

- **Análise de meta-heurísticas para o problema de roteamento de veículos com várias restrições**
  - *Autor(es)/Ano:* Vítor Gauer Pereira (Orientador: Omir Correia Alves Júnior) / 2019
  - *Link:* [https://repositorio.udesc.br/entities/publication/a4f6518b-150b-4d55-9c32-1ee4baa3818a](https://repositorio.udesc.br/entities/publication/a4f6518b-150b-4d55-9c32-1ee4baa3818a)
