...
printf("Insira o endereço(cidade):\n");
...
void procurar_salario(struct pessoas *a, int x, float y){
for(int i=0; i<x; i++){
 if( (a[i].salario) == y ){
printf("Nome: %s\n", (a[i].nome));
printf("Data de nascimento: %d/%d/%d\n", a[i].x.dia, a[i].x.mes, a[i].x.ano);
...
case 3:
float sal;
printf("Digite o salario da pessoa que esta buscando:\n");
scanf("%f", &sal);
procurar_salario(a, num, sal);
break;
